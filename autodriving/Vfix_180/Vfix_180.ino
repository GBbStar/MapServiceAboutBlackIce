...
void setup() 
{
  ...
  //myFile=SD.open("test015.txt",FILE_WRITE);
  //myFile.close();
...
void loop() 
{
  //if(mySerial.available()){Serial.write(mySerial.read())};//블루투스측 내용을 시리얼 모니터에 출력
  //if(mySerial.available()){mySerial.write(Serial.read())};//시리얼 모니터 내용을 블루투스 측에 출력
  digitalWrite(motorBR_a,LOW);//alow_bhigh=straight
  digitalWrite(motorBR_b,HIGH);
  analogWrite(motorBR_Control,speed_U);//속도 고정(이후에는 새로 변수 설정해서 함수 도입, 새 스케치 오픈)
  digitalWrite(motorBL_a,HIGH);//ahigh_blow=straight
  digitalWrite(motorBL_b,LOW);
  analogWrite(motorBL_Control,speed_U);
  IRR_value=analogRead(IR_R); //핀번호 맞추기
  IRL_value=analogRead(IR_L); //핀번호 맞추기
...
//if(IRL_VALUE=111 && IRL_VALUE=111)~
