...
void setup() 
{
  ...
  //myFile=SD.open("test013.txt",FILE_WRITE);
  //myFile.close();
...
void loop() 
{
  ...
  for (deinde=1;deinde<101;deinde++)
  {
    if(deinde>=0 && deinde<=30)
    {
      hey=deinde;
      int speed_K= -hey*3+220; //205~80
    ...
    else if(deinde>30 && deinde<=70)
    {
      hey=deinde;
      int speed_K= 130+(hey-30)*2;
      digitalWrite(motorBR_a,LOW);//alow_bhigh=straight
      digitalWrite(motorBR_b,HIGH);
      analogWrite(motorBR_Control,speed_K);//속도 고정(이후에는 새로 변수 설정해서 함수 도입, 새 스케치 오픈)
      digitalWrite(motorBL_a,HIGH);//ahigh_blow=straight
